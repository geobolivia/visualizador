...
if (leyenda_param != "on" && capas_param == "on" && var_aux=="existe") {
  ...
  // esta se aumento
  alto_param =alto_param - height_switch -0.15*height_switch  + var_aux ;
...
document.write("<div style='visibility:" + visibilidad_leyenda+
               ";top:" + top_leyenda +
               ";height:" + height_leyenda +
               ";width:" + width_leyenda +
               ";left:" + left_leyenda +
               ";' id='layerswitcher'> </div>");

document.write("<div style='position:absolute;top:" + top_mapa +
               "; left:" + left_mapa +
               "; width:" + ancho_param +
               "; height:" + alto_param +
               "; padding:10px' id='map'></div>");

document.write("<div style='top:5 ; width:" + "200" +
               "; left:" + "110"+
               ";' id='medidores'> </div>");

document.write("<div style='visibility:" + visibilidad_switcher +
               ";top:" + top_switch +
               "; left:" + left_switch +
...
document.write("<div style='position:absolute;top:" + top_control +
               "' id='options' ><div id='output'></div>");
...
function init() {
  ...
  //Aqui se introdujo el parametro wmc_param que es el nombre del archivo wmc
  //que viene en la url

  // MIENTRAS NO ARREGLEMOS EL PROXY, PARA TRABAJAR DESCOMENTAR LAS LINEAS 278 Y
  //390, Y COMENTAR LA 279 Y LA 391
  ...
  if (merge) {
    ...
    try {
      ...
      var mapOptions = jsonFormat.read(
        OpenLayers.Util.getElement('mapOptions').value
      );
      map = format.read(text, {
        map: mapOptions
      });

      //numero de layers que contiene el WMC
      var numeroLayers = map.baseLayer.div.parentElement.childElementCount
      var nombreMostrar;
      var metadato;
      var nombreLayer;
        ...
        for (var i = 0; i < numeroLayers; i++) {
          if (layer.name == map.layers[i].params.LAYERS) {
            if (layer.attribution != null) {
              ...
              geobol_ArrayInfo1[i] = '<p class="p_layerinfo">' + geobol_ArrayInfo1[i] + "<a href=" + pagweb + "> (" + titulo + ")</a>";
